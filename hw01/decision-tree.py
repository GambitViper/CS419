...
def decision_tree_learning(examples, attributes, parent_examples, depth):
    # print(f"depth: {depth}")
...
def test_classification_equality(examples):
    ...
    for ex in examples[1: (len(examples)-1)]:
        # print(f"testing classification : {ex.classification()} =? {classification}")
        if(ex.classification() != classification):
            # print(f"found a counter-example")
            check = False
    return check
...
def entropy(poisin, edible):
    ...
    total = poisin + edible
    if(total != 0):
        poisin_coef = poisin / total
        edible_coef = edible / total
        # print(f"( P:[{poisin} | {total}] E:[{edible} | {total}] )")
        if(poisin_coef != 0):
...
results = []
for curr_incr in range(t_incr, t_size + t_incr, t_incr):
    curr_training_set = []
    for ct in range(curr_incr):
        idx = pick_random_idx(len(training_set))
        curr_training_set.append(training_set[idx])
    
    decision_tree = decision_tree_learning(curr_training_set, list(range(21)), [], 0)
...
for result in results:
    print(f"Training set size: {printicr}.  Success:  {result} percent.")
    printicr += t_incr
